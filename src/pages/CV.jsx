...
function CV() {
  ...
  const [selectV, setSelectV] = useState(oneMonthLater);
  ...
  return (
    <Layout>
      ...
      <div className="w-full h-[calc(100vh-145px)] items-center md:items-start bg-gray-200 flex flex-col md:grid grid-cols-3 grid-rows-2 justify-center md:justify-start gap-3 p-3">
        {ankets.map((anket, index) => {
          return (
            <div
              key={index}
              className="bg-white hover:shadow-lg h-full rounded shadow w-full cursor-pointer p-3 scale-cus transition-all flex flex-col justify-between"
            >
              <div>
                <div className="w-full">
                  ...
                  <div className="nunito-600 mt-3 truncate">
                    {anket.jobTitle}
                  </div>
                  <div className="nunito-600 text-[13px] truncate text-gray-500 mt-2 text-clip ">
                    {anket.jobDuty}
                  </div>
                </div>
